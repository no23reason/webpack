...
class ConcatenatedModule extends Module {
	...
	/**
	 * @param {object} ctx generate context
	 * @param {DependencyTemplates} ctx.dependencyTemplates dependency templates
	 * @param {RuntimeTemplate} ctx.runtimeTemplate runtime template
...
class HarmonyImportSpecifierDependencyConcatenatedTemplate {
	...
	harmonyInit(dep, source, { runtimeTemplate, dependencyTemplates }) {
		...
		const info = this.modulesMap.get(module);
		if (!info) {
			this.originalTemplate.harmonyInit(dep, source, {
				runtimeTemplate,
				dependencyTemplates
			});
			return;
	...
	apply(dep, source, { runtimeTemplate: runtime, dependencyTemplates }) {
		const module = dep._module;
		const info = this.modulesMap.get(module);
		if (!info) {
			this.originalTemplate.apply(dep, source, {
...
class HarmonyImportSideEffectDependencyConcatenatedTemplate {
	...
	harmonyInit(dep, source, { runtimeTemplate: runtime, dependencyTemplates }) {
		const module = dep._module;
		const info = this.modulesMap.get(module);
		if (!info) {
			this.originalTemplate.harmonyInit(dep, source, {
