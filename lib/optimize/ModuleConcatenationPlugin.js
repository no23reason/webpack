				...
				compilation.hooks.optimizeChunkModules.tap(
					...
					(chunks, modules) => {
						...
						for (const concatConfiguration of concatConfigurations) {
							...
							// Work around issue by using a standard for loop and assigning moduleGraph.getModule(dep).reasons
							for (let i = 0; i < newModule.dependencies.length; i++) {
								let dep = newModule.dependencies[i];
								if (moduleGraph.getModule(dep)) {
									let reasons = moduleGraph.getModule(dep).reasons;
