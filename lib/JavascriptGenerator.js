...
class JavascriptGenerator extends Generator {
	 ...
	 * @param {RenderContext} renderContext the renderContext
	...
	sourceBlock(
		...
		const { dependencyTemplates, runtimeTemplate } = renderContext;
		...
		if (vars.length > 0) {
			...
			if (varStartCode && varEndCode) {
				const start = block instanceof AsyncDependenciesBlock && block.range ? block.range[0] : -10;
				const end = block instanceof AsyncDependenciesBlock && block.range
					? block.range[1]
	 ...
	 * @returns {void}
	 */
	sourceDependency(dependency, renderContext, source) {
		const constructor =
			/** @type {new (...args: any[]) => Dependency} */ (dependency.constructor);
		const template = renderContext.dependencyTemplates.get(constructor);
		if (!template) {
			throw new Error(
				"No template for dependency: " + dependency.constructor.name
			);
		}
		template.apply(dependency, source, renderContext);
	}
